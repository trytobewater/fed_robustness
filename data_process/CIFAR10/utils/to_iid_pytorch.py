...
def partition_data(dataset, datadir, n_nets, alpha=0.5):
    ...
    for k in range(K):
        ...
        if k == 0:
            ...
            id_train = np.concatenate((id_train, idx_k))
            id_test = np.concatenate((id_test, idx_k_test))
    ...
    num_elems = N
    test_num_elems = N_test

    group_size = int(N/n_nets) # 每个client上有多少张图片
    test_group_size = int(N_test/n_nets) # 每个client上有多少张图片

    num_big_groups = num_elems - n_nets * group_size # 0
    num_small_groups = n_nets - num_big_groups # 100

    test_num_big_groups = test_num_elems - n_nets * test_group_size  # 0
    test_num_small_groups = n_nets - test_num_big_groups  # 100

    for i in range(num_small_groups):
        idx_batch[i].append(id_train[group_size * i : group_size * (i + 1)])

    for i in range(test_num_small_groups):
        idx_batch_test[i].append(id_test[test_group_size * i : test_group_size * (i + 1)])


    bi = group_size*num_small_groups
    ci = test_group_size*test_num_small_groups
    group_size += 1
    for i in range(num_big_groups):
        idx_batch[i].append(id_train[bi + group_size * i: bi + group_size * (i + 1)])
    for i in range(test_num_big_groups):
        idx_batch_test[i].append(id_test[ci + test_group_size * i: ci + test_group_size * (i + 1)])
    ...
    #         # proportions = np.random.dirichlet(np.repeat(alpha, n_nets))# np.repeat(alpha, n_nets)是100个0.5
    #         proportions = np.random.uniform(np.repeat([0.1, 1], n_nets))
    #         print(proportions)
    #
    #         ## Balance
...
for client_id in range(args.total_clients):
    ...
    #501
    with open('/data/yc/leaf_te_data/cifar10/data/iid_train/train_'+ str(client_id + 1) + '.json', 'w') as outfile:
        json.dump(train_data, outfile)
    #
    with open('/data/yc/leaf_te_data/cifar10/data/iid_test/test_'+ str(client_id + 1) + '.json', 'w') as outfile:
