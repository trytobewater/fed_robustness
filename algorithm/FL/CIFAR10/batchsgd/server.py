...
class Server:
    ...
    def savecheckpoint(self, epoch, flag):
        dir_modify_here = 'normal-'
        state = {
            'net': self.float_model.state_dict()
        }
        if epoch > 0 and flag > 1:
            os.remove('./checkpoint/normal/point/' + dir_modify_here + str(epoch - 1) + '.pth')
        torch.save(state, './checkpoint/normal/point/' + dir_modify_here + str(epoch) + '.pth')
    ...
    def update_model(self, clients, model_type='int', epoch=0):
        ...
        if model_type == 'int':
            ...
            self.update_SGD_float_model(epoch)
        else:
