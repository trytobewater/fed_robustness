...
def main():
    ...
    data_dir_modify_here_1 = 'train_react_60_noise_0.6'

    print(data_dir_modify_here_1)
    data_dir_modify_here_2 = 'test_react_60_noise_0.6'

    train_data_dir = os.path.join('./', 'data', data_dir_modify_here_1)
    test_data_dir = os.path.join('./', 'data', data_dir_modify_here_2)
    ...
    np.save('result-niti-float-avg-bn-femnist-1.npy', plot_result)
...
def generate_model(model_type='int'):
    if model_type == 'int':
        ...
        if args.dataset == 'mnist' and args.model == 'lenet':
            print('we choose mnist on lenet')
            model = lenet().to('cuda:0')

        elif args.dataset == 'mnist' and args.model == 'vgg':
            model = VGG_cifar(16, 62).to('cuda:0')
...
def setup_clients(model_type='int', train_data_loader=None, test_data_loader=None):
    ...
    for key in train_data_loader:
        u = key

        train_data = train_data_loader[u]

        test_data = test_data_loader[u]
