...
class Client:
    ...
    def train(self, epoch, cids, num_epochs=1, batch_size=5, model_type='int'):
        ...
        data_loader = cifar10_somebatch_loaders('train', self.train_data)
        ...
        uptate_para = self.model.state_dict()

        layer_name_list_old, layer_val_list_old = list(init_para.keys()), list(init_para.values())
        layer_name_list_new, layer_val_list_new = list(uptate_para.keys()), list(uptate_para.values())

        gradients = []
        for i in range(len(layer_val_list_old)):
            gradients.append(layer_val_list_new[i] - layer_val_list_old[i])

        return losses.avg, gradients
                    ...
                    if args.model_type == 'int':
                        for idx, l in enumerate(model.forward_layers):
                            if hasattr(l, 'weight'):
                                ...
                                writer.add_histogram('Grad/' + l.__class__.__name__ + '_' + str(idx), grad,
                                                     epoch * total_steps + i)

                    elif args.model_type == 'float':
                        for idx, l in enumerate(model.layers):
                            if hasattr(l, 'weight'):
                                writer.add_histogram('Grad/' + l.__class__.__name__ + '_' + str(idx), l.weight.grad,
                                                     epoch * total_steps + i)
