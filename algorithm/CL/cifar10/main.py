...
train_data_dir = os.path.join(root_pa, 'yc', 'leaf_new_data', 'cifar10', 'data',
                              'notrans_train_64')
test_data_dir = os.path.join(root_pa, 'yc', 'leaf_new_data', 'cifar10', 'data', 'notrans_test_64')
...
net = VGG_cifar(depth=16, num_classes=10)
...
def train(epoch):
    ...
    Train_Loss.append(train_loss / (100))
...
# this version includes adding GN
# for epoch in range(start_epoch, start_epoch + 200):
#     if epoch < 20:
#         train(epoch)
#         test(epoch)
...
#             a.append(net.layers[i])
#             if isinstance(net.layers[i], nn.Conv2d):
#                 num_channels = net.layers[i].out_channels
#                 a.append(nn.GroupNorm(num_channels, num_channels))
#         net.layers = nn.Sequential(*a)
#         net = net.to(device)
#         train(epoch)
#         test(epoch)
#     else:
