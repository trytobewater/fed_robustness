...
def _load_image(train_or_test, img_name):
    data_dir_modify_here_1 = 'train_react_60_light_0.1-10' #train_need_data

    data_dir_modify_here_2 = 'test_react_60_light_0.1-10' #test_react_60_rotation_60

    if train_or_test:

        IMAGES_DIR = os.path.join('/data', 'yc','leaf_new_data','celeba','data', data_dir_modify_here_1)
        img = Image.open(os.path.join(IMAGES_DIR, img_name))
        img = img.resize((84, 84)).convert('RGB')
        return np.array(img)
    else:
        IMAGES_DIR = os.path.join('/data', 'yc','leaf_new_data','celeba',  'data',data_dir_modify_here_2)
        img = Image.open(os.path.join(IMAGES_DIR, img_name))
        img = img.resize((84, 84)).convert('RGB')
        return np.array(img)
...
def cifar10_loaders(train_or_test, data={'x' : [],'y' : []}):
    ...
    transform_train = transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)),  # 对数据按0通道进行标准化，即先减均值，再除以标准差
    ])
...
def cifar10_singlebatch_loaders(train_or_test, data_x, data_y):
    ...
    if train_or_test == 'train':
        trainset = CIFAR10_single_batch_truncated(data_x, data_y, batch_size, train=True, transform=transform_train)
